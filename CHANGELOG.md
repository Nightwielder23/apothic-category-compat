...
- `weapon_pickaxes_as_melee` config flag in `apothic_compat-common.toml` (default true), gating the weapon pickaxes override through a custom data map condition and matching the 1.20.1 and 1.19.2 `weapon_pickaxes_as_heavy` option. Set it false to leave those items in Apotheosis's native breaker category.
