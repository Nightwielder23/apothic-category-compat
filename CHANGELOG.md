...
- Weapon pickaxes tag and overrides. L_Ender's Cataclysm Void Forge and Infernal Forge and the Forbidden Arcanus blacksmith gavels extend the pickaxe class, so Apotheosis sorted them into its breaker (mining) category ahead of melee and they rolled mining gem bonuses on items used as weapons. They now map to melee weapon through the `apothic_category_compat:weapon_pickaxes` item tag, which a datapack can extend.
- `weapon_pickaxes_as_melee` config flag in `apothic_category_compat-common.toml` (default true), gating the weapon pickaxes override through a custom data map condition and matching the 1.20.1 and 1.19.2 `weapon_pickaxes_as_heavy` option. Set it false to leave those items in Apotheosis's native breaker category.
