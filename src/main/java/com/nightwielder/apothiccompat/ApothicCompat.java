...
public class ApothicCompat {
    ...
    // the last mod-loading event, after all such deferred init, so re-run the same dispatch here. The IMC
    ...
    private void reapplyAfterDeferredInit(FMLLoadCompleteEvent event) {
        ...
        LOGGER.info("Apothic Compat second pass re-categorized {} item(s) after deferred mod init.", changed);
    }
