...
When an item is set in both `adventure.cfg` and Apothic Category Compat (the config file or a compat module), Apothic Category Compat's value currently wins. To keep an `adventure.cfg` value for such an item, leave that item out of `apothic_category_compat-common.toml`.
